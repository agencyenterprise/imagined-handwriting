            ...
            for i, (prefix, (p_b, p_nb)) in enumerate(beam):  # Loop over beam
                ...
                if lm is not None:
                    ...
                    if str_prefix == ">":
                        n_p_nb = NEG_INF
                    # UPDATE
                    # only score if we have completed a word, i.e. we are adding a space
                    elif str_prefix[-1] in [">", "~", "?", ","]:
                        str_prefix = str_prefix.replace(">", " ").replace("~", ".")
                        # print('prefix', str_prefix, 'split', str_prefix.split(" "))
                        score = lm.score(
                            str_prefix, eos=False, bos=True
                        )  # -log10 score
                        # add a penalty for really long words
                        # current_word_len = len(str_prefix.split(" ")[-2])
                        # if current_word_len > 0:
                        #    long_word_penalty_score = (
                        #        long_word_penalty * math.log(current_word_len)
                        #    )
                        # else:
                        #    long_word_penalty_score = 0

                        # print(long_word_penalty)
                        # print(lm_weight * score)
                        n_p_nb += lm_weight * score  # - long_word_penalty_score
                ...
                if trie is not None:
                    ...
                    # remove punctuation at the end of the prefix so we can end
                    # with punctuation
                    if len(word_prefix) > 2 and word_prefix[-1] in ["~", "?", ",", "'"]:
                        word_prefix = word_prefix[:-1]

                    # do not allow punctuation in the middle of the word
                    # this is a hack that puts a space in the middle of the word
                    # so the trie can find it
                    word_prefix = (
                        word_prefix.replace("~", " ")
                        .replace("?", " ")
                        .replace(",", " ")
                    ...
                    # if len(word_prefix)>0 and word_prefix[-1] in ['~', '?', ',']:
                    #     word_prefix = word_prefix[:-1]
